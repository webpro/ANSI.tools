// AUTO-GENERATED from src/tokenize.ts. Do not edit.
    ...
    if (state === 0) {
      ...
      if (is8BitIntroducer(charCode)) {
        ...
        if (nextCode === CSI_OPEN) {
          ...
          result.push({ type: TOKEN_TYPES.INTRODUCER, pos: i, raw: input.substring(i, nextPos + 1), code: input[nextPos] });
          i = nextPos + 1;
          state = 1;
          ...
          if (intermediate) {
            ...
            result.push({ type: TOKEN_TYPES.INTRODUCER, pos: i, raw: input.substring(i, j), code: ESC_CODE });
          }
          i = j;
          if (j < len) {
            state = 1;
      ...
      if (currentCode === CSI) {
        ...
        if (state === 1 && i > dataStart)
          result.push({ type: TOKEN_TYPES.DATA, pos: dataStart, raw: input.substring(dataStart, i) });
